/**
 * Test: Run OpenCode using executeSessionCommand
 *
...
async function main() {
  console.log("=== executeSessionCommand Background Method (OpenCode) ===\n")
  ...
  try {
    ...
    await sandbox.process.executeSessionCommand(sessionId, {
      command: `export OPENCODE_PERMISSION='{"*":"allow"}'`,
    })
    console.log("   Environment set.\n")

    // 5. Start opencode with runAsync: true
    console.log("5. Starting OpenCode with runAsync: true...")
    const outputFile = "/tmp/opencode-session-output.jsonl"
    const pidFile = "/tmp/opencode-session.pid"
    ...
    const command = `(script -q -c "ANTHROPIC_API_KEY='${apiKey}' OPENCODE_PERMISSION='{\"*\":\"allow\"}' opencode run --format json --variant medium '${prompt}'" ${outputFile}; echo 1 > ${outputFile}.done) & echo $! > ${pidFile}`
    ...
    console.log("\n=== executeSessionCommand Method Complete (OpenCode) ===")
