/**
 * Test: Run OpenCode using executeCommand
 *
...
async function main() {
  console.log("=== executeCommand Background Method (OpenCode) ===\n")
  ...
  try {
    ...
    // 3. Start opencode with nohup (returns immediately)
    console.log("3. Starting OpenCode with nohup...")
    const outputFile = "/tmp/opencode-output.jsonl"
    const prompt = "Write a hello world Python script and run it"
    const nohupCommand = `nohup sh -c 'opencode run --format json --variant medium "${prompt}" >> ${outputFile} 2>&1; echo 1 > ${outputFile}.done' > /dev/null 2>&1 & echo $!`
    ...
    console.log("\n=== executeCommand Method Complete (OpenCode) ===")
