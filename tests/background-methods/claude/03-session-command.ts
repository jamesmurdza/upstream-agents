...
async function main() {
  ...
  try {
    ...
    // Use script -q -c to create a PTY, which is required for Claude CLI to output properly
    // NOTE: runAsync + Claude requires PTY simulation via 'script' command because Claude buffers output without TTY
    const command = `(script -q -c "ANTHROPIC_API_KEY='${apiKey}' claude --print --output-format stream-json --verbose --dangerously-skip-permissions '${prompt}'" ${outputFile}; echo 1 > ${outputFile}.done) & echo $! > ${pidFile}`
